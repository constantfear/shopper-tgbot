...
from Lexicon.lexicon import LEXICON_MESSAGE, LEXICON_KEYBOARD, PRODUCT_HEADER, ORDER_HEADER, ORDER_LIST_HEADER
...
async def proc_start_command(message: Message, state: FSMContext):
    ...
    products = get_products_by_type(product_type).all()
...
async def add_product(message: Message, state: FSMContext):
    product_types = get_types()
    msg = msg = LEXICON_MESSAGE['add_product_type']+''.join([str(row[0])+'. '+row[1]+'\n' for row in product_types])
    await message.answer(text = msg, parse_mode='HTML', reply_markup=empty_keyboard)
...
async def show_orders(message: Message, state: FSMContext):
    ...
    msg = func.to_table_message(ORDER_HEADER, orders)
...
async def show_orders(message: Message, state: FSMContext):
    await message.answer(text = LEXICON_MESSAGE['get_order_id'], parse_mode='HTML', reply_markup=cancel_panel)
...
async def proc_start_command(message: Message, state: FSMContext):
    ...
    order_list = get_order_list(order_id).all()
    await state.update_data(order_id = order_id)
    msg = func.to_table_message(ORDER_LIST_HEADER, order_list)
    if msg:
...
async def show_orders(message: Message, state: FSMContext):
    try: 
        ...
        await message.answer(text = LEXICON_MESSAGE['changed_order_status'], parse_mode='HTML', reply_markup=admin_panel)
    except:
...
async def exit_admin(message: Message, state: FSMContext):
    await message.answer(text = LEXICON_MESSAGE['exit'], parse_mode='HTML', reply_markup=main_panel)
    await state.clear()
